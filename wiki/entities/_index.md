---
type: meta
title: "Entities Index"
created: 2026-06-21
updated: 2026-06-21
tags:
  - meta
  - index
status: stable
related:
  - "[[Wiki Index]]"
---

# Entities

People, organizations, products, repositories, and tools.

## People
- [[GhostKellz]] — owner; MSP operator, systems engineer, OSS author

## Organizations
- [[CK Technology LLC]] — the MSP

## Sites
- [[ckelley.dev]] — portfolio / dev site
- [[ghostkellz.sh]] — GPG identity landing
- [[cktechx.com]] — MSP business site

## Ghost Ecosystem — repositories
- [[Bolt]] — container runtime (Rust)
- [[GhostChain]] — blockchain stack (Rust)
- [[ghostbrew]] — package tooling
- [[ghostcrate]] — crate/registry tooling
- [[ghostctl]] — all-in-one sysadmin/DevOps/homelab toolkit (Rust)
- [[GhostForge]] — build/forge tooling
- [[Ghostlang]] — embeddable scripting engine (Zig)
- [[ghostls]] — Ghostlang language server (Zig)
- [[GhostPanel]] — control panel
- [[ghostshell]] — shell tooling
- [[ghostsnap]] — snapshot tooling
- [[GhostView]] — viewer
- [[ghostVK]] — Vulkan helper
- [[Ghostwarden]] — guard/security tooling
- [[GhostWave]] — audio/signal tooling
- [[GhostWire]] — networking/mesh (Rust)
- [[Grim]] — editor (Zig)
- [[Grove]] — tree-sitter/parsing (Zig)
- [[GShell]] — scriptable shell (Zig)
- [[Phantom]] — TUI/framework (Zig)
- [[PhantomLink]] — linking layer
- [[Wraith]] — tooling
- [[zcrypto]] — crypto library (Zig)
- [[zquic]] — QUIC implementation (Zig)
- [[zqlite]] — embedded SQLite-style DB (Zig)
- [[zsync]] — sync library (Zig)
- [[Jarvis]] — agentic AI assistant (Rust)
- [[Zeke]] — AI tooling (Rust)
- [[reaper]] — tooling

## NV Tools Suite — repositories
- [[nvcontrol]] — NVIDIA control (Rust)
- [[nvbind]] — NVIDIA container binding (Rust)
- [[nvhud]] — overlay/HUD
- [[nvlatency]] — latency tooling
- [[nvproton]] — Proton integration
- [[nvshader]] — shader tooling
- [[nvsync]] — sync tooling
- [[nvvk]] — Vulkan helper
- [[Proton-NV]] — Proton/NVIDIA integration

## Tools & external products
- [[Ollama]] — local LLM runtime
- [[Restic]] — backup tool
- [[Snapper]] — Btrfs snapshot manager
- [[Tailscale]] — mesh VPN
- [[Unbound]] — recursive DNS resolver
- [[KDE Plasma]] — desktop environment
